...
IniData <- function(Granularity = 'H', TradingParts ='US', DDay, DDayMinus, DDayPlus) {
  
  
  # Hourly US model
  if (Granularity == 'H' & TradingParts =='US') {
    Data <- data.frame('date' = c(DDayMinus, rep(DDay,24), DDayPlus), 'time' = c(2300, seq(0, 2300, by = 100), 0), load_act = NaN, load_fcst = NaN) 
  } 
  
  # HH US Model
  else if (Granularity == 'HH' & TradingParts =='US') {
  ...
  # QH US Model
  else if (Granularity == 'QH' & TradingParts =='US') {
  ...
  else if (Granularity == 'TM' & TradingParts =='US') {
...
CreateTableQuery <- function(TradingParts, DbIdStats, StatsTable) {
  ...
  ifelse(TradingParts == 'UK',
         ...
         # US TEMPLATE 
         paste0("CREATE [", DbIdStats,"].[dbo].[", StatsTable, "]
                (
                Date varchar(50) NOT NULL PRIMARY KEY,
                Week_Number float(53) NULL,
                Week_Day varchar(50) NULL,
...
CalculateStats <- function(DbTZ, Data, InstantLoad, DDay, DDayMinus, DDayPlus, CompareTime, StatsTable, DbIdStats, ErrorType = "NOTWX") {
  ...
  if(DbTZ == "UTC-5") {
    TimeZoneDb ="Etc/GMT+5"
    TimezoneStats = "US/Eastern"
    ...
    TimeZoneDb = "GMT+6"
    TimezoneStats = "US/Central"
    
  } else if (DbTZ == "UTC-7")  {
    TimeZoneDb ="Etc/GMT+7"
    TimezoneStats = "US/Mountain" 
    
  } else if (DbTZ == "UTC-8")  {
    TimeZoneDb ="Etc/GMT+8"
    TimezoneStats = "US/Pacific" 
...
GetStats <- function(ModelId , DbId = NULL, LoadActTable = NULL, LoadRollTable = NULL , StatsTable = NULL,
                     DaysAhead = 1, CompareTime = "09:30", SolveDays = 1:10, 
                     Granularity = 'H', DbSvr = usdbsvr9, DbSvrStats = usdbsvr9,  DbIdStats = "tesla",
                     DbTZ = 'UTC-5', TradingParts = 'US', CreateTable = F, InstantLoad = F, BadDays = NULL, TSO = F) {
  ...
  if(is.null(StatsTable)) { StatsTable <- paste0(ifelse(TSO == T, "ISO_", "TESLA_"), ModelId,"_<X>_DA")}
  if(is.null(LoadActTable)) { LoadActTable <- paste0(ModelId, "_load_act")}
  if(is.null(LoadRollTable)) { LoadRollTable <- paste0(ModelId, ifelse(TSO == T,"_iso",""), "_load_fcst_r")}
  ...
  for (DA in DaysAhead ) {
    ...
    for (d in SolveDays) {
      ...
      # Get Days
      DDay <- as.numeric(as.character.Date(Sys.time()-d*86400, format = "%Y%m%d"))
      DDayMinus  <- as.numeric(as.character.Date(Sys.time()-(d+1)*86400, format = "%Y%m%d"))
      DDayPlus <- as.numeric(as.character.Date(Sys.time()-(d-1)*86400, format = "%Y%m%d"))
      ...
      # Calculate stats 
      ################################
      UpdateQuery <- ifelse(TradingParts == "US", 
...
GetWXStats <- function(ModelId , ErrorType, DbId = NULL, LoadActTable = NULL, LoadRollTable = NULL , LoadBackcastTable = NULL, StatsTable = NULL,
                       DaysAhead = 1, CompareTime = "09:30", SolveDays = 1:10, 
                       Granularity = 'H', DbSvr = usdbsvr9, DbSvrStats = usdbsvr9,  DbIdStats = "tesla",
                       DbTZ = 'UTC-5', TradingParts = 'US', CreateTable = T, InstantLoad = F, BadDays = NULL) {
